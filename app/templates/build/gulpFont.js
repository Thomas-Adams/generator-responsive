...
var ignoreFiles = function(file, stats) {
...
var executeFontCreation = function(allChars, originalFontFile, generatedFontFile) {
    ...
    var inputFontFile = originalFontFile ? originalFontFile : path.join(__dirname, '../fonts/HYb2gj.ttf');
    ...
    //args.push('--canonical-order');
    //args.push('--legacy-cmap');
    //args.push('--flavor=ttf');
    //args.push("--drop-tables-='BASE','JSTF','DSIG','EBDT','EBLC','EBSC','SVG','PCLT','LTSH','Feat','Glat','Gloc','Silf','Sill','CBLC','CBDT','sbix','COLR','CPAL'");
...
var processFilesRecursively = function(startDirectory, opts) {
    recursive(startDirectory, [ignoreFiles], function(err, files) {
        ...
        executeFontCreation(allChars, opts.originalFontFile, opts.generatedFontFile);
    });
};

var gulpFont = function(startDirectory, options) {
    if (!fs.existsSync(startDirectory)) {
        new gutil.PluginError({
            plugin: 'Font',
            message: 'Start directory "' + startDirectory + '" cannot be found.'
        });
    }

    var opts = _.extend({
        originalFontFile: './fonts/BMWTypeChinese.ttf',
        generatedFontFile: './fonts/generated/subsetted.ttf',
        matchTags: [{
            tag: "h2",
    ...
    return through.obj(function(file, enc, callback) {
        ...
        processFilesRecursively(startDirectory, opts);
        this.push(file);
        return callback();
    });

};
module.exports = gulpFont;
